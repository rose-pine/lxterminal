<p align="center">
    <img src="https://github.com/rose-pine/rose-pine-theme/raw/main/assets/icon.png" width="80" />
    <h2 align="center">Rosé Pine for LXTerminal</h2>
</p>

<p align="center">All natural pine, faux fur and a bit of soho vibes for the classy minimalist</p>

<p align="center">
    <a href="https://github.com/rose-pine/rose-pine-theme">
        <img src="https://img.shields.io/badge/community-rosé%20pine-26233a?labelColor=191724&logo=data:image/svg+xml;base64,&style=for-the-badge" />
    </a>
</p>

## Usage

1. Create a backup of your existing LXTerminal config file present at `~/.config/lxterminal/lxterminal.conf`.
2. Browse to the `[general]` section in your active `lxterminal.conf` file and delete the lines corresponding to the color settings, starting from `bgcolor` to `color_preset`.
3. Browse to the file for your choice of flavor (named as `rose-pine.conf`, `rose-pine-dawn.conf` or `rose-pine-moon.conf`) inside the `dist` folder.
4. Copy the contents of this file and paste them in your active `lxterminal.conf` file.
5. Restart LXTerminal to apply the changes.

## Gallery

Fetch featured below is [NerdFetch](https://github.com/thatonecalculator/nerdfetch)

**Rosé Pine**

![Rosé Pine with LXTerminal](https://user-images.githubusercontent.com/108054324/204729678-23bb546a-63e0-4b3c-b96a-bbbad7ef8859.png)

**Rosé Pine Dawn**

![Rosé Pine Dawn with LXTerminal](https://user-images.githubusercontent.com/108054324/204729789-12dbb28d-19af-4736-a87f-d5e653080c68.png)

**Rosé Pine Moon**

![Rosé Pine Moon with LXTerminal](https://user-images.githubusercontent.com/108054324/204729908-c3c8fe81-4c1d-4fca-93de-586d88c165d1.png)

## Thanks to 

- [ThatOneCalculator](https://github.com/thatonecalculator)
- [Prateek](https://github.com/prateektade)

## Contributing

Modify `template.conf` using Rosé Pine variables, then build variants:

```
npx @rose-pine/build@latest -t template.conf
```

_Generated by @rose-pine/build@0.5.1_
